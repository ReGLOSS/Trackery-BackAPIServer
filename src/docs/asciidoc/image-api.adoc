...
사용자가 업로드한 이미지의 시도 커버리지를 조회하는 API입니다.
...
===== 요청
include::{snippets}/get-image-sido-coverage-success/http-request.adoc[]
...
=== 내 이미지 시도별 시군구 커버리지 조회 API
==== GET `/api/images/me/coverage/sido/+{sidoId}+`

사용자가 업로드한 이미지의 시도별 시군구 커버리지를 조회하는 API입니다.
시도의 포함된 시군구 중 이미지가 한 장이라도 업로드 된 시군구 ID를 배열로 반환합니다.

===== 요청
include::{snippets}/get-image-sigungu-coverage-success/http-request.adoc[]

===== 응답
include::{snippets}/get-image-sigungu-coverage-success/http-response.adoc[]
include::{snippets}/get-image-sigungu-coverage-success/response-fields.adoc[]
'''
