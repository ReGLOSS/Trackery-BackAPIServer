...
사용자의 프로필 이미지를 업데이트 합니다.

===== 요청
include::{snippets}/update-profile-image/http-request.adoc[]
include::{snippets}/update-profile-image/request-fields.adoc[]

===== 응답
include::{snippets}/update-profile-image/http-response.adoc[]
include::{snippets}/update-profile-image/response-fields.adoc[]
'''
...
===== 처리 과정
1. 사용자 정보 익명화 (이메일, 사용자명, 닉네임을 랜덤 식별자로 변경, 비밀번호 무작위화)
2. OAuth 연동 정보 완전 삭제
3. 액세스 토큰 블랙리스트 등록
4. 리프레시 토큰 Redis에서 삭제
5. 클라이언트 쿠키 삭제 (컨트롤러에서 처리)
